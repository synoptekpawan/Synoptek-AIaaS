...
class churnUtility:
    ...
    def calculateDateDiff(self,date,max_date):
    ...
    def calculateDistinct_X(self,df1, key_column,X_column):
        
        df2=pd.DataFrame(df1.groupby(key_column)[X_column].nunique()).reset_index()
        return df2
    ...
    """
    def calculateSum_X(self,df1, key_column,X_column):
        df2=pd.DataFrame(df1.groupby(key_column)[X_column].sum()).reset_index()
        return df2
    ...
    def calculateStDev_X(self,df1, key_column, X_column):
        df2=pd.DataFrame(df1.groupby(key_column)[X_column].std()).reset_index()
        return df2
    """
    ...
    def calculateNumericalDataFeaturesForGroup(self,df1,key_column,  summable_columns,max_date, rename_label):
        ...
        for feat in summable_columns:
            if feat not in exclude_list:
                ...
                feat_dfs.append(df_temp)
        df_final = reduce(lambda left,right: pd.merge(left,right,on=key_column, how='outer'), feat_dfs)
    ...
    def calculateNumericalDataFeatures(self,df1,key_column, summable_columns, rename_label, IsDevelopment=True):
        ...
        if IsDevelopment==True:
            ...
            end_date=max_date
        print(f'Training Period Start Date:{start_date}, End Date: {end_date}, Days Excluded:{days_to_exclude}')
        df_temp=df1

        df_current=df_temp.loc[df_temp['Parsed_Date'].between(start_date, end_date)]
        print(f'CalculateNumericalDataFeatures :  shape:{df_current.shape}')
        df_num=self.calculateNumericalDataFeaturesForGroup(df_current,key_column, summable_columns, end_date,rename_label='Overall-')
    ...
    def calculateStringDataFeatures(self,df1,key_column, uniquable_columns,  rename_label, IsDevelopment=True):
        base_features_list=[key_column]
        df_profile=df1[base_features_list].drop_duplicates()
        churn_period = self.churn_period
        ...
        if IsDevelopment==True:
            ...
            end_date=max_date
        print(f'Training  Start Date:{start_date} ,  end date: {end_date}')
        #df_current=df_temp
        df_current=df_temp.loc[df1['Parsed_Date'].between(start_date, end_date)]
        df_str=self.calculateStringDataFeaturesForGroup(df_current,key_column, uniquable_columns, rename_label='Overall-')
        df_str.fillna(0,inplace=True)
        print('df_str.shape ', df_str.shape)
                    ...
                    if denom_feat not in exclude_list :
                        if 'Period' in num_feat or 'Period' in denom_feat:
                            if 'Period' in num_feat and 'Period' in denom_feat:
                                print(f'Calculating {num_feat}_ratio_{denom_feat}')
                                num_prefix=num_feat.split('_')[0]#.split('-')[1]
                                denom_prefix=denom_feat.split('_')[0]#.split('-')[1]
                                
                                if num_prefix== denom_prefix:
                                    print(f'Calculating {num_feat}_ratio_{denom_feat}')
                                    df_final[num_feat+'_per_'+denom_feat]=df_final[num_feat]/(df_final[denom_feat]+1.0)
    ...
    def assign_churn_status_to_group(self,df, min_date,max_date,  key_column, activity_column):
        
        churn_period = self.churn_period
        churn_threshold = self.churn_threshold
        
        # Calculate the activity for each subscriber in the overall period
        df_overall=pd.DataFrame(df.groupby(key_column)[activity_column].count()).reset_index().copy()
        ...
        df_prechurn=pd.DataFrame(df[df['Parsed_Date'].between(min_date,max_date-np.timedelta64(days,'D'))].groupby(key_column)[activity_column].count()).reset_index().copy()
        ...
        df_prechurn=pd.merge(df_overall,df_prechurn,on=key_column, how='outer')
        df_prechurn['diff_products_purchased']=df_prechurn.OverallProductsPurchased-df_prechurn.PrechurnProductsPurchased
        
        
        # Assigning the Churner and Nonchurner labels
        df_prechurn['Label']=np.where(df_prechurn.diff_products_purchased<=churn_threshold,'Churner','Nonchurner')

        # Removing extra columns from the dataframe
        to_keep=[each for each in df_prechurn.columns if each not in ['OverallProductsPurchased','PrechurnProductsPurchased','diff_products_purchased']]
    ...
    def assign_churn_status(self, df, key_column, activity_column):
        ...
        if (interval_end - np.timedelta64(min_reqd_period,'D'))>=interval_start:
            df_labeled=self.assign_churn_status_to_group(df, interval_start,interval_end,key_column,activity_column)
            return df_labeled
        else:
